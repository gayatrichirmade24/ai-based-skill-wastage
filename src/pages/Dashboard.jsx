...
function wastageMeta(score) {
  if (score <= 30) return { label: "Good", color: "#16a34a", bg: "#dcfce7", border: "#86efac", text: "0-30% wastage means your current occupation uses most of your skills." };
  if (score <= 60) return { label: "Moderate", color: "#a16207", bg: "#fef9c3", border: "#fde68a", text: "31-60% wastage means some useful skills are not being used in your current role." };
  return { label: "High Wastage", color: "#dc2626", bg: "#fee2e2", border: "#fca5a5", text: "61-100% wastage means your current occupation does not use many skills you possess." };
}

function fitMeta(score) {
  if (score >= 80) return { label: "Strong Utilization", color: "#16a34a", bg: "#dcfce7", border: "#86efac", text: "80%+ utilization means your current occupation uses your skill profile well." };
  if (score >= 55) return { label: "Medium Utilization", color: "#a16207", bg: "#fef9c3", border: "#fde68a", text: "55-79% utilization means your role uses some skills but leaves room for better alignment." };
  return { label: "Weak Utilization", color: "#dc2626", bg: "#fee2e2", border: "#fca5a5", text: "Below 55% utilization means many of your skills are not visible in your current occupation." };
}

function buildResumeImprovements(report) {
  const raw = report.raw || {};
  const missing = raw.missing_skills || [];
  const unused = raw.unused_skills || report.skills.filter((s) => s.status !== "active").map((s) => s.name);
  const matched = raw.matched_skills || report.skills.filter((s) => s.status === "active").map((s) => s.name);
  const ideas = [];

  if (missing.length) {
    ideas.push(`Add one project or certification that proves ${missing.slice(0, 2).join(" and ")}.`);
  }
  if (unused.length) {
    ideas.push(`Show ${unused.slice(0, 2).join(" and ")} through projects or consider career roles where these skills are actively required.`);
  }
  if (matched.length) {
    ideas.push(`Strengthen matched skills like ${matched.slice(0, 2).join(" and ")} with numbers, outcomes, or links.`);
  }

  ideas.push("Use action-result bullets such as: Built a dashboard that reduced manual reporting time by 30%.");
  ideas.push("Add GitHub, portfolio, or live demo links for your strongest projects.");
...
function normalizeRecommendations(items = []) {
  return items.map((item, index) => {
    if (typeof item === "string") {
      return {
        type: index === 0 ? "Career Direction" : "Action Step",
        priority: index < 2 ? "High" : "Medium",
        title: item.split(":")[0] || "Recommended action",
        action: item,
        why: "This action is based on your skill wastage and utilization pattern.",
        impact: index < 2 ? "High" : "Medium",
      };
    }
    return {
      type: item.type || "Action Step",
      priority: item.priority || "Medium",
      title: item.title || "Recommended action",
      action: item.action || item.why || "",
      why: item.why || "This action is based on your skill analysis.",
...
function priorityStyle(priority) {
  if (priority === "High") return { bg: "#fee2e2", color: "#dc2626", border: "#fca5a5" };
  if (priority === "Medium") return { bg: "#fef9c3", color: "#a16207", border: "#fde68a" };
...
export default function Dashboard() {
  ...
  const jobFitScore = Number(
    report.raw?.job_fit_score ||
    report.jobFitScore ||
    report.roleMatches?.[0]?.match ||
    0
  );
  const wastageInfo = wastageMeta(Number(report.wastageIndex || 0));
  const fitInfo = fitMeta(jobFitScore);
  const resumeImprovements = buildResumeImprovements(report);
  const aiRecommendations = normalizeRecommendations(report.recommendations);
  ...
  const downloadReportPdf = () => {
    const matched = report.raw?.matched_skills || report.skills.filter((s) => s.status === "active").map((s) => s.name);
    const missing = report.raw?.missing_skills || [];
    const unused = report.raw?.unused_skills || report.skills.filter((s) => s.status !== "active").map((s) => s.name);
    ...
    win.document.write(`
      <html>
        <head>
          <title>Skill Wastage Report</title>
          <style>
            ...
            .card { border: 1px solid #dbeafe; border-radius: 10px; padding: 14px; background: #f8faff; }
            .score { font-size: 28px; font-weight: 800; color: #2563eb; }
            ul { margin-top: 8px; }
            @media print { button { display: none; } body { padding: 12px; } }
          </style>
        </head>
        <body>
          <button onclick="window.print()" style="padding:10px 16px;border:0;border-radius:8px;background:#2563eb;color:white;font-weight:700;float:right">Save as PDF</button>
          <h1>AI Skill Wastage Report</h1>
          <p class="muted">${displayName} | ${report.fileName || "Uploaded Resume"}</p>
          <div class="grid">
            <div class="card"><div class="score">${report.wastageIndex}%</div><strong>Skill Wastage</strong><p>${wastageInfo.label}</p></div>
            <div class="card"><div class="score">${jobFitScore}%</div><strong>Role Utilization</strong><p>${fitInfo.label}</p></div>
            <div class="card"><div class="score">${report.totalSkills}</div><strong>Total Skills</strong><p>${report.activeCount} active</p></div>
          </div>
          <h2>Matched Skills</h2><p>${matched.join(", ") || "No matched skills found."}</p>
          <h2>Missing Skills</h2><p>${missing.join(", ") || "No major missing skills found."}</p>
          <h2>Unused Skills</h2><p>${unused.join(", ") || "No unused skills found."}</p>
          <h2>Recommendations</h2><ul>${aiRecommendations.map((rec) => `<li><strong>${rec.title}:</strong> ${rec.action}</li>`).join("")}</ul>
          <h2>Resume Improvements</h2><ul>${resumeImprovements.map((idea) => `<li>${idea}</li>`).join("")}</ul>
        </body>
      </html>
    `);
    win.document.close();
        ...
        <div style={{ maxWidth: 1160, margin: "0 auto", padding: "40px 40px 60px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 4 }}>
            {[
              ...
              { label: "Role Utilization", val: jobFitScore + "%",       color: "#4ade80" },
            ...
            <button onClick={downloadReportPdf} style={{ border: "1px solid rgba(255,255,255,.35)", background: "rgba(255,255,255,.14)", color: "#fff", borderRadius: 14, padding: "14px 18px", fontFamily: "inherit", fontWeight: 800, cursor: "pointer" }}>
              Download Report PDF
            </button>
          </div>
        </div>
        ...
        {tab === "overview" && (
          <div style={{ animation: "fadeUp 0.4s ease both" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 18 }}>
              <div style={{ ...S.card, border: "1px solid " + wastageInfo.border, background: wastageInfo.bg }}>
                <p style={{ ...S.label, color: wastageInfo.color }}>Wastage Explanation</p>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 14, alignItems: "center" }}>
                  <div>
                    <h3 style={{ margin: "0 0 6px", color: wastageInfo.color, fontSize: "1rem" }}>{wastageInfo.label}</h3>
                    <p style={{ margin: 0, color: "#334155", fontSize: ".82rem", lineHeight: 1.55 }}>{wastageInfo.text}</p>
                  </div>
                  <strong style={{ color: wastageInfo.color, fontSize: "1.7rem", fontFamily: "monospace" }}>{report.wastageIndex}%</strong>
                </div>
              </div>
              <div style={{ ...S.card, border: "1px solid " + fitInfo.border, background: fitInfo.bg }}>
                <p style={{ ...S.label, color: fitInfo.color }}>Current Occupation Explanation</p>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 14, alignItems: "center" }}>
                  <div>
                    <h3 style={{ margin: "0 0 6px", color: fitInfo.color, fontSize: "1rem" }}>{fitInfo.label}</h3>
                    <p style={{ margin: 0, color: "#334155", fontSize: ".82rem", lineHeight: 1.55 }}>{fitInfo.text}</p>
                  </div>
                  <strong style={{ color: fitInfo.color, fontSize: "1.7rem", fontFamily: "monospace" }}>{jobFitScore}%</strong>
                </div>
              </div>
            </div>

            <div style={{ ...S.card, marginBottom: 18 }}>
              <p style={S.label}>Resume Improvement Suggestions</p>
              <div style={{ display: "grid", gap: 10 }}>
                {resumeImprovements.map((idea, i) => (
                  <div key={idea} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    <div style={{ width: 24, height: 24, borderRadius: 7, background: "#eff6ff", color: "#2563eb", display: "flex", alignItems: "center", justifyContent: "center", fontSize: ".7rem", fontWeight: 900, flexShrink: 0 }}>{i + 1}</div>
                    <p style={{ margin: 0, color: "#475569", fontSize: ".84rem", lineHeight: 1.55 }}>{idea}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ ...S.card, marginTop: 18 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {aiRecommendations.map((rec, i) => {
                  ...
                  return (
                  <div key={rec.title + i} style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: 14, background: "#fff", display: "grid", gap: 8 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start" }}>
                      <div>
                        <p style={{ fontSize: ".7rem", color: "#2563eb", fontWeight: 800, textTransform: "uppercase", letterSpacing: ".08em", margin: "0 0 4px" }}>{rec.type}</p>
                        <h4 style={{ margin: 0, color: "#0f172a", fontSize: ".92rem" }}>{rec.title}</h4>
                      </div>
                      <span style={{ fontSize: ".66rem", padding: "3px 8px", borderRadius: 999, background: ps.bg, color: ps.color, border: "1px solid " + ps.border, fontWeight: 800, whiteSpace: "nowrap" }}>{rec.priority}</span>
                    </div>
                    <p style={{ fontSize: ".82rem", color: "#475569", lineHeight: 1.55, margin: 0 }}>{rec.action}</p>
                    <p style={{ fontSize: ".74rem", color: "#94a3b8", lineHeight: 1.45, margin: 0 }}>{rec.why}</p>
                  </div>
                  );
                })}
              </div>
          ...
          <div style={{ animation: "fadeUp 0.4s ease both", display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ ...S.card, background: "#eff6ff", border: "1px solid #bfdbfe" }}>
              <div style={{ display: "flex", gap: 12 }}>
                ...
                <div>
                  <p style={{ fontSize: "0.9rem", fontWeight: 700, color: "#1d4ed8", marginBottom: 4 }}>Alternative Career Path Recommendations</p>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div>
                      ...
                      {r.missingSkills.length > 0 && (
                        <p style={{ fontSize: "0.72rem", color: "#d97706", margin: "5px 0 0" }}>
                          Improve next: {r.missingSkills.slice(0, 3).join(", ")}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: "0.82rem", color: "#0c4a6e", lineHeight: 1.55, margin: 0 }}>
                    These actions will reduce your wastage score and improve career alignment. Recommended based on your actual skill gaps.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div style={{ ...S.card, border: "1px solid #bbf7d0" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(155px,1fr))", gap: 11 }}>
                {[
                  ...
                  { period: "Month 3", focus: "Apply for better-fit roles or freelance work",       bg: "#f0fdf4", c: "#15803d", border: "#bbf7d0" },
                ].map((p) => (
