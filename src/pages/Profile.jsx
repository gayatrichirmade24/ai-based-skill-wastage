...
export default function Profile() {
  ...
  const [saved, setSaved]   = useState(() => isProfileComplete(readSavedProfile()));
  ...
  const validate = () => {
    ...
    if (!profile.job.trim())             e.job        = "Current role is required";
    ...
    if (!profile.about.trim())           e.about      = "About yourself is required";
    setErrors(e);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validate()) {
      try { localStorage.setItem("userProfile", JSON.stringify(profile)); } catch { /* Profile can still continue if browser storage is unavailable. */ }
        ...
        <div className="top-grid" style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20 }}>
          ...
          <div style={S.card}>
            ...
            {saved ? (
              ...
              <form onSubmit={handleSubmit} style={{ animation: "fadeUp .35s ease" }}>
                ...
                <Row label="Current Role" error={errors.job}>
                ...
                <Row label="Location" error={errors.location}>
                  <input style={S.input} type="text" value={profile.location}
                    onChange={e => setProfile({ ...profile, location: e.target.value })}
                    placeholder="City, Country" />
                </Row>
                <Row label="About Yourself" error={errors.about}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ ...S.card, padding: "20px" }}>
              ...
              {[
                { icon: "📄", title: "Upload your resume",        desc: "PDF upload supported with automatic text extraction."          },
                { icon: "🔍", title: "NLP skill extraction",      desc: "AI reads and extracts every skill automatically." },
                { icon: "📊", title: "Occupation skill matching",  desc: "Your profile and resume skills are compared with your current role."       },
