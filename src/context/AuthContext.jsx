...
async function syncBackendUser(user, fallbackName) {
  if (!user) return;
  try {
    await fetch(`${API_BASE}/register`, {
      ...
      body: JSON.stringify({
        name: user.displayName || fallbackName || "User",
        email: user.email,
        firebase_uid: user.uid,
      }),
    });
  } catch {
    // Firebase auth should still work if the local backend is not running yet.
  }
}
...
export function AuthProvider({ children }) {
  ...
  const registerWithEmail = async (name, email, password) => {
    ...
    await syncBackendUser(cred.user, name);
    return cred;
  };
